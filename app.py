...
@st.cache_data
def dolggagggi(goal_level, hexa_frag_cost, hexa_prob, sunday_maple, frag_price, reset_cost, high_value_preference, num_frags, mode):
    dol_result = {}
    ...
    # 조각 소모
    if mode:
        dol_result['num_frags'] = num_frags
    ...
    # 조각 무한 멸망전
        ...
        dol_result['expected_cost'] = expected_cost
        dol_result['expected_frag_cost'] = expected_frag_cost
    ...
    dol_result['chart_data'] = chart_data
...
with factors:
    with st.container(border=True):
        ...
        if st.button('깎기'):
            ...
            dol_result = dolggagggi(goal_level, hexa_frag_cost, hexa_prob, sunday_maple, frag_price, reset_cost, high_value_preference, num_frags, st.session_state['frag_mode'])

            for key, value in dol_result.items():
